...
class RTCMBits:
    ...
    def calcRTCMPosition(self, satinfo, msgsatid, msgprc, scalefactors):
        ...
        for i in range(msgsatcnt):
            ...
            if scalefactors[i] == 1:
                err *= 16.0
            if svid not in satinfo.prSmoothed:
                continue
    ...
    def RTCMType1(self, satinfo, maxsats=32):
        ...
        for svid in satinfo.prSmoothed:
            prAdjusted = satinfo.prSmoothed[svid] + satinfo.receiver_clock_error * util.speedOfLight + \
                         satinfo.satellite_clock_error[svid] * util.speedOfLight
            ...
            if svid not in self.error_history:
    ...
    def RTCMType1_step(self, throttle=True):
        ...
        scalefactors = []
        for svid in self.error_history:
            if svid not in self.iode or svid not in errors:
                continue
